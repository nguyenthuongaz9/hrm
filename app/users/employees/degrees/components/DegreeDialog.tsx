...
export default function DegreeDialog({
    ...
    const handleDeleteButton = () => {
        if (variant === 'delete') {
            axios.delete(`/api/degrees/${data.id}`, {
                data: {
                    id: data.id
                }
            }).then((callback) => {
    ...
    useEffect(() => {
        if (data && variant === 'edit') {
            form.setValue('name', data.degreeName);
                ...
                <DialogContent className='w-[35rem]'>
                    ...
                    <div className='space-y-4 mt-4'>
                        <div className='flex flex-col gap-2'>
                            <h3 className='text-md font-bold'>Tên bằng cấp</h3>
                            <p className='border rounded-md px-3 py-3'>{data?.degreeName}</p>
                        </div>
                        <div className='flex flex-col gap-2'>
    ...
    if (variant === 'delete') {
        return (
            <Dialog className='' open={isOpen}>
                <DialogContent >
                    <h3>Bạn chắc chắn muốn xóa bằng cấp này?</h3>
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input placeholder="Tên bằng cấp" {...field} />
                                        </FormControl>
