...
interface KindOfDisciplineDialogProps {
    isOpen: boolean;
...
const formSchema = z.object({
    name: z.string().min(1, {
        message: "Tên loại kỉ luật không được để trống"
    }),
})
...
export default function KindOfDisciplineDialog({
    isOpen,
    setIsOpen,
    variant,
    data
}: KindOfDisciplineDialogProps) {
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        if (variant === 'create') {
            axios.post('/api/disciplines/kindOfDiscipline',{
                ...values
            }).then((callback)=>{
        ...
        if (variant === 'edit') {
            axios.put(`/api/disciplines/kindOfDiscipline/${data.id}`,{
                ...values
            }).then((callback) => {
                if (callback.status === 200) {
    ...
    const handleDeleteButton = () => {
        if (variant === 'delete') {
            axios.delete(`/api/disciplines/kindOfDiscipline/${data.id}`, {
                data: {
                    id: data.id
                }
            }).then((callback) => {
    ...
    if (variant === 'delete') {
        return (
            <Dialog className='' open={isOpen}>
                <DialogContent >
                    <h3>Bạn chắc chắn muốn xóa loại kỉ luật?</h3>
        ...
        <Dialog open={isOpen} >
            ...
            <DialogContent className='w-[35rem]'>
                ...
                <div className='w-full flex justify-center items-center'>
                    <h3 className='text-md font-bold'>{variant === 'edit' ? 'Chỉnh sửa loại kỉ luật' : 'Thêm loại kỉ luật'}</h3>
                </div>
                <div>
                    <Form {...form}>
