...
interface KindOfRewarDialogProps {
    isOpen: boolean;
...
const formSchema = z.object({
    name: z.string().min(1, {
        message: "Tên loại khen thưởng không được để trống"
    }),
})
...
export default function KindOfRewarDialog({
    isOpen,
    setIsOpen,
    variant,
    data
}: KindOfRewarDialogProps) {
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        if (variant === 'create') {
            axios.post('/api/rewards/kindOfReward',{
                ...values
            }).then((callback)=>{
        ...
        if (variant === 'edit') {
            axios.put(`/api/rewards/kindOfReward/${data.id}`,{
                ...values
            }).then((callback) => {
                if (callback.status === 200) {
    ...
    const handleDeleteButton = () => {
        if (variant === 'delete') {
            axios.delete(`/api/rewards/kindOfReward/${data.id}`, {
                data: {
                    id: data.id
                }
            }).then((callback) => {
    ...
    if (variant === 'delete') {
        return (
            <Dialog className='' open={isOpen}>
                <DialogContent >
                    <h3>Bạn chắc chắn muốn xóa loại khen thưởng?</h3>
